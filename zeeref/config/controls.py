...
from zeeref.config.settings import ZeeSettings, settings_events
...
class MouseConfigBase:
    ...
    def set_modifiers(self, value):
        logger.debug(
            f'Setting {self.SETTINGS_GROUP} modifiers for "{self.id}" to: {value}'
        )
    ...
    def set_inverted(self, value):
        logger.debug(
            f'Setting {self.SETTINGS_GROUP} inverted for "{self.id}" to: {value}'
        )
...
class MouseWheelConfig(MouseConfigBase):
    ...
    def controls_changed(self):
        ...
        return (
            set(self.get_modifiers()) != set(self.modifiers)
...
class MouseConfig(MouseConfigBase):
    ...
    def set_button(self, value):
        logger.debug(
            f'Setting {self.SETTINGS_GROUP} button for "{self.id}" to: {value}'
        )
        self.kb_settings.set_value(
    ...
    def matches_event(self, event):
        ...
        modifiers = self.get_modifiers()
        return (
            self.modifiers_to_qt(modifiers) == event.modifiers()
...
class KeyboardSettings(QtCore.QSettings):
    ...
    def __init__(self):
        ...
        filename = os.path.join(
            os.path.dirname(ZeeSettings().fileName()), "KeyboardSettings.ini"
        )
