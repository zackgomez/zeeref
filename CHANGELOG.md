...
- Added a confirmation dialog when attempting to close unsaved files.
  The confirmation dialog can be disabled in:
...
- Shortcuts now only trigger once when holding down the key
  combination to avoid inconsistent program states and potential
...
- Arrange Horizontal/Vertical now also sort by filename instead of
...
- Moving the window from within ZeeRef now changes to a different cursor from
...
- Keyboard shortcuts can now be edited from within ZeeRef (Settings ->
...
- Alternative way to move the ZeeRef window without the title bar:
...
- The shortcut to move the ZeeRef window (Ctrl + Alt + Drag)
...
Note that bee files from version 0.2.0 won't open in ZeeRef 0.1.x.
