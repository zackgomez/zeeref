...
def test_scale_items_by_ignore_first_redo(qapp):
    ...
    command = commands.ScaleItemsBy(
        [item1, item2], 2, QtCore.QPointF(100, 100), ignore_first_redo=True
    )
    command.redo()
    assert item1.scale() == 1
    assert item1.pos().x() == 0
...
def test_rotate_items_by(qapp):
    ...
    item2.setPos(100, 100)
    item2.do_flip()
    command = commands.RotateItemsBy([item1, item2], -90, QtCore.QPointF(100, 100))
    command.redo()
...
def test_rotate_items_by_ignore_first_redo(qapp):
    ...
    item2.setPos(100, 100)
    item2.do_flip()
    command = commands.RotateItemsBy(
        [item1, item2], -90, QtCore.QPointF(100, 100), ignore_first_redo=True
    )
    command.redo()
    assert item1.rotation() == 0
    assert item1.pos().x() == 0
...
def test_flip_items_horizontal(qapp):
    ...
    item2.setPos(100, 100)
    item2.do_flip()
    command = commands.FlipItems(
        [item1, item2], QtCore.QPointF(100, 100), vertical=False
    )
    command.redo()
    assert item1.flip() == -1
    assert item1.rotation() == 0
...
def test_flip_items_vertical(qapp):
    ...
    item2.do_flip()
    command = commands.FlipItems(
        [item1, item2], QtCore.QPointF(100, 100), vertical=True
    )
    command.redo()
    assert item1.flip() == -1
