...
from zeeref.widgets.controls.keyboard import (
...
def test_keyboard_shortcuts_editor_on_save_no_conflicts(view):
    ...
    with patch("zeeref.widgets.controls.keyboard.actions", ActionList([a1, a2])):
...
def test_keyboard_shortcuts_editor_on_save_conflicts_cancel(question_mock, view):
    a1 = Action(id="foo", text="Foo", shortcuts=["Ctrl+F"])
    a2 = Action(id="bar", text="Bar", shortcuts=["Ctrl+B"])
    with patch("zeeref.widgets.controls.keyboard.actions", ActionList([a1, a2])):
        editor = KeyboardShortcutsEditor(
...
def test_keyboard_shortcuts_editor_on_save_conflicts_confirm(question_mock, view):
    a1 = Action(id="foo", text="Foo", shortcuts=["Ctrl+F"])
    a2 = Action(id="bar", text="Bar", shortcuts=["Ctrl+B"])
    with patch("zeeref.widgets.controls.keyboard.actions", ActionList([a1, a2])):
        editor = KeyboardShortcutsEditor(
...
@patch(
    "zeeref.widgets.controls.keyboard.actions",
    ActionList(
        [
...
def test_keyboard_shortcuts_model_data_gets_text():
    ...
    with patch("zeeref.widgets.controls.keyboard.actions", ActionList([action])):
        model = KeyboardShortcutsModel()
        value = model.data(
            index=MagicMock(
...
def test_keyboard_shortcuts_proxy_setdata_saves_correct_filtered_index():
    ...
    with patch("zeeref.widgets.controls.keyboard.actions", ActionList([a1, a2, a3])):
        proxy = KeyboardShortcutsProxy()
        proxy.setFilterFixedString("b")
        proxy.setData(
            index=proxy.index(1, 2), value=QtGui.QKeySequence("Ctrl+B"), role=None
        )
