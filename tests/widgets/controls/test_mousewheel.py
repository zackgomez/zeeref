...
from zeeref.widgets.controls.mousewheel import (
    MouseWheelDelegate,
...
def test_mousewheel_editor_inits_modifiers_when_not_configured(view):
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=[], invertible=True
    )
    with patch(
        "zeeref.config.controls.KeyboardSettings.MOUSEWHEEL_ACTIONS",
        ActionList([action]),
    ):
...
def test_mousewheel_editor_inits_modifiers_when_configured(view):
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt", "Ctrl"], invertible=True
    )
    with patch(
        "zeeref.config.controls.KeyboardSettings.MOUSEWHEEL_ACTIONS",
        ActionList([action]),
...
def test_mousewheel_editor_on_modifiers_changed_when_a_modifier_checked(view):
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["No Modifier"], invertible=True
    )
    with patch(
        "zeeref.config.controls.KeyboardSettings.MOUSEWHEEL_ACTIONS",
        ActionList([action]),
...
def test_mousewheel_editor_get_temp_action(view):
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=True
    )
    with patch(
        "zeeref.config.controls.KeyboardSettings.MOUSEWHEEL_ACTIONS",
        ActionList([action]),
...
@patch("zeeref.widgets.controls.mousewheel.MouseWheelModifiersEditor.accept")
def test_mousewheel_editor_on_save_no_conflicts(accept_mock, view):
    a1 = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=True
    )
    a2 = MouseWheelConfig(
        id="bar1", group="bar", text="Bar", modifiers=["Ctrl"], invertible=True
    )
...
@patch("zeeref.widgets.controls.mousewheel.MouseWheelModifiersEditor.accept")
def test_mousewheel_editor_on_save_reenter_existing_shortcut(accept_mock, view):
    a1 = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=True
    )
    a2 = MouseWheelConfig(
...
@patch("zeeref.widgets.controls.mousewheel.MouseWheelModifiersEditor.accept")
def test_mousewheel_editor_on_save_conflicts_cancel(accept_mock, msg_mock, view):
    a1 = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=True
    )
    a2 = MouseWheelConfig(
...
@patch("zeeref.widgets.controls.mousewheel.MouseWheelModifiersEditor.accept")
def test_mousewheel_editor_on_save_conflicts_confirm(accept_mock, msg_mock, view):
    a1 = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=True
    )
    a2 = MouseWheelConfig(
...
def test_mousewheel_delegate_setmodeldata(view):
    ...
    with patch(
        ...
        editor.set_modifiers(["Ctrl", "Shift"])

    with patch.object(
        editor, "result", return_value=QtWidgets.QDialog.DialogCode.Accepted
    ):
        delegate.setModelData(editor, model, index=model.index(0, 2))
...
def test_flags_inverted_column_when_not_invertible():
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Alt"], invertible=False
    )
...
def test_mousewheel_model_data_gets_text():
    ...
    value = model.data(
        index=MagicMock(
            column=MagicMock(return_value=0), row=MagicMock(return_value=0)
        ),
...
def test_mousewheel_model_data_gets_changed_when_not_changed():
    ...
    value = model.data(
        index=MagicMock(
            column=MagicMock(return_value=1), row=MagicMock(return_value=0)
        ),
        role=QtCore.Qt.ItemDataRole.DisplayRole,
    )
...
def test_mousewheel_model_data_gets_modifiers():
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Ctrl", "Alt"], invertible=False
    )
    ...
    value = model.data(
        index=MagicMock(
            column=MagicMock(return_value=2), row=MagicMock(return_value=0)
        ),
        role=QtCore.Qt.ItemDataRole.DisplayRole,
    )
...
def test_mousewheel_model_data_gets_inverted_when_invertible():
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=["Ctrl", "Alt"], invertible=True
    )
    ...
    value = model.data(
        index=MagicMock(
            column=MagicMock(return_value=3), row=MagicMock(return_value=0)
        ),
        role=QtCore.Qt.ItemDataRole.DisplayRole,
    )
    assert value == "No"
...
def test_mousewheel_model_data_tooltip_modifiers_changed_from_not_configured():
    action = MouseWheelConfig(
        id="foo1", group="foo", text="Foo", modifiers=[], invertible=False
    )
...
def test_mousewheel_proxy_data_unfiltered():
    ...
    a3 = MouseWheelConfig(
        id="baz1", group="baz", text="Baz", modifiers=["Shift"], invertible=True
    )
    ...
    assert proxy.data(proxy.index(0, 0), QtCore.Qt.ItemDataRole.DisplayRole) == "Bar"
    assert proxy.data(proxy.index(1, 0), QtCore.Qt.ItemDataRole.DisplayRole) == "Foo"
    assert proxy.data(proxy.index(2, 0), QtCore.Qt.ItemDataRole.DisplayRole) == "Baz"
...
def test_mousewheel_proxy_data_filtered():
    ...
    assert proxy.data(proxy.index(0, 0), QtCore.Qt.ItemDataRole.DisplayRole) == "Bar"
    assert proxy.data(proxy.index(1, 0), QtCore.Qt.ItemDataRole.DisplayRole) == "Baz"
...
def test_mousewheel_proxy_setdata_saves_correct_filtered_index():
    ...
    proxy.setFilterFixedString("b")
    proxy.setData(
        index=proxy.index(1, 3), value={"modifiers": ["Ctrl", "Alt"]}, role=None
    )
