...
from zeeref.config.controls import (
    KeyboardSettings,
    MouseConfig,
    MouseWheelConfig,
)
...
def test_mousewheelconfig_get_modifiers_default(kbsettings):
    action = MouseWheelConfig(
        id="foo", group="foobar", text="Foo", modifiers=["Shift"], invertible=False
    )
...
def test_mousewheelconfig_modifiers_to_single():
    assert MouseWheelConfig.modifiers_to_qt(["Alt"]) == Qt.KeyboardModifier.AltModifier


def test_mousewheelconfig_controls_changed_not_changed():
    action = MouseWheelConfig(
        id="foo", group="foobar", text="Foo", modifiers=["Shift"], invertible=True
    )
...
def test_mousewheelconfig_conflicts_with_true():
    ...
    action2 = MouseWheelConfig(
        id="bar", group="foobar", text="Bar", modifiers=["Shift"], invertible=True
    )
...
def test_mousewheelconfig_conflicts_false_when_both_not_configured():
    ...
    action2 = MouseWheelConfig(
        id="bar", group="foobar", text="Bar", modifiers=[], invertible=True
    )
    assert action1.conflicts_with(action2) is False


def test_mousewheelconfig_matches_event_true():
    event = MagicMock(modifiers=MagicMock(return_value=Qt.KeyboardModifier.AltModifier))
    action = MouseWheelConfig(
        id="foo", group="foobar", text="Foo", modifiers=["Alt"], invertible=True
    )
...
def test_mousewheelconfig_matches_event_false():
    event = MagicMock(modifiers=MagicMock(return_value=Qt.KeyboardModifier.AltModifier))
    action = MouseWheelConfig(
        id="foo", group="foobar", text="Foo", modifiers=["Ctrl"], invertible=True
    )
...
@patch("zeeref.config.KeyboardSettings.setValue")
@patch("zeeref.config.KeyboardSettings.remove")
