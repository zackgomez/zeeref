...
from zeeref.config.settings import CommandlineArgs
...
@patch("zeeref.config.settings.parser.parse_args")
...
def test_settings_remove_with_callback(settings):
    foo_callback = MagicMock()
    settings.FIELDS = {"foo/bar": {"default": 66, "post_save_callback": foo_callback}}
    settings.remove("foo/bar")
...
def test_settings_recent_files_get_existing_only(settings):
    with tempfile.NamedTemporaryFile() as f:
        settings.update_recent_files("foo.zref")
...
def test_settings_recent_files_update(settings):
    settings.update_recent_files("foo.zref")
    settings.update_recent_files("bar.zref")
    assert settings.get_recent_files() == [
        os.path.abspath("bar.zref"),
        os.path.abspath("foo.zref"),
    ]
...
def test_settings_recent_files_update_respects_max_num(settings):
    for i in range(15):
        settings.update_recent_files(f"{i}.zref")

    recent = settings.get_recent_files()
    assert len(recent) == 10
    assert recent[0] == os.path.abspath("14.zref")
    assert recent[-1] == os.path.abspath("5.zref")
