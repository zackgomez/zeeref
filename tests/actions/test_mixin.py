...
from zeeref.actions.menu_structure import MENU_SEPARATOR
...
@patch("zeeref.config.KeyboardSettings.get_list")
def test_create_actions(kb_mock, toggle_mock, trigger_mock, qapp):
    ...
    from zeeref.actions.mixin import actions
...
def test_create_actions_with_shortcut_from_settings(qapp, kbsettings):
    with patch(
        "zeeref.actions.mixin.actions",
        ActionList(
            [Action(id="foo", text="&Foo", shortcuts=["Ctrl+F"], callback="on_foo")]
        ),
    ):
...
def test_create_actions_checkable_checked_true(toggle_mock, trigger_mock, qapp):
    widget = FooWidget()
    widget.build_menu_and_actions()
    trigger_mock.connect.assert_not_called()
    toggle_mock.connect.assert_called_once_with(widget.on_foo)
...
def test_create_actions_checkable_with_settings(
    ...
    widget.build_menu_and_actions()
    settings_mock.value.assert_called_once_with("foo/bar", False, type=bool)
    qaction = widget.actions()[0]
    assert qaction.isChecked() is True
...
@patch(
    "zeeref.actions.mixin.menu_structure", [{"menu": "Foo", "items": [MENU_SEPARATOR]}]
)
@patch("zeeref.actions.mixin.actions", ActionList([]))
...
@patch(
    "zeeref.actions.mixin.menu_structure",
...
@patch("zeeref.actions.mixin.actions", ActionList([]))
def test_create_recent_files_more_than_10_files(kb_mock, triggered_mock, qapp):
    kb_mock.side_effect = lambda group, key, default: default
    widget = FooWidget()
    widget.settings.get_recent_files.return_value = [
        os.path.abspath(f"{i}.zref") for i in range(15)
    ]
...
@patch("zeeref.actions.mixin.actions", ActionList([]))
@patch("zeeref.config.KeyboardSettings.get_list")
def test_create_recent_files_fewer_files_than_10_files(kb_mock, triggered_mock, qapp):
    kb_mock.side_effect = lambda group, key, default: default
    widget = FooWidget()
    widget.settings.get_recent_files.return_value = [
        os.path.abspath(f"{i}.zref") for i in range(5)
    ]
...
def test_update_recent_files(triggered_mock, qapp):
    widget = FooWidget()
    widget.settings.get_recent_files.return_value = [os.path.abspath("foo.zref")]

    widget.build_menu_and_actions()
    triggered_mock.connect.reset_mock()
    assert len(widget.actions()) == 1
    qaction1 = widget.actions()[0]
    assert qaction1.text() == "foo.zref"

    widget.settings.get_recent_files.return_value = [os.path.abspath("bar.zref")]
    widget.update_menu_and_actions()
    triggered_mock.connect.assert_called()
    assert len(widget.actions()) == 1
    qaction1 = widget.actions()[0]
    assert qaction1.text() == "bar.zref"
