...
def test_selection_action_items(scene):
    ...
    assert action_items == {item1, item2, scene.multi_select_item}
...
def test_lower_behind_selection_when_selection(scene):
    ...
    scene.multi_select_item.setZValue(5)
    scene.multi_select_item.lower_behind_selection()
    assert scene.multi_select_item.zValue() == 2.999
...
def test_lower_behind_selection_when_no_selection(scene):
    ...
    scene.multi_select_item.setZValue(5)
    scene.multi_select_item.lower_behind_selection()
    assert scene.multi_select_item.zValue() == 5
...
def test_mouse_press_event_when_ctrl_leftclick(mouse_mock):
    ...
    event = MagicMock(
        ...
        modifiers=MagicMock(return_value=Qt.KeyboardModifier.ControlModifier),
    )
