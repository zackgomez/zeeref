...
# Build the ZeeRef appimage. Run from the git root directory.
# On github actions:
...
parser = argparse.ArgumentParser(
    description=("Create an appimage for ZeeRef. Run from the git root directory.")
)
parser.add_argument(
    "-v", "--version", required=True, help="ZeeRef version number/tag for output file"
)
parser.add_argument(
    "-j",
    "--jsonfile",
...
parser.add_argument(
    "--skip-apt", default=False, action="store_true", help="Skip apt install step"
)
parser.add_argument(
    "-l",
    "--loglevel",
...
url = (
    "https://github.com/niess/python-appimage/releases/download/"
...
try:
    ...
    pass
run_command("./python.appimage", "--appimage-extract", capture_output=True)

run_command(
    "squashfs-root/usr/bin/pip",
    "install",
    ".",
...
runbee = f'"$APPDIR/opt/python{PYVER}/bin/python{PYVER}" -I -m zeeref "$@"'
logfile = "/tmp/ZeeRefAppimageLog.txt"
...
content.extend(
    [
        ...
        # See also https://github.com/rbreu/zeeref/issues/102
...
filename = "org.zeeref.ZeeRef.appdata.xml"
...
Terminal=false
Exec=ZeeRef-{BEEVERSION}
Type=Application
Icon=logo

MimeType=application/x-zeeref;
Categories=Qt;KDE;Graphics;
X-KDE-NativeMimeType=application/x-zeeref
X-KDE-ExtraNativeMimeTypes=

X-AppImage-Version={BEEVERSION}
"""

filename = "squashfs-root/usr/share/applications/org.zeeref.ZeeRef.desktop"
with open(filename, "w") as f:
    f.write(content)
os.symlink(
    "usr/share/applications/org.zeeref.ZeeRef.desktop", "squashfs-root/ZeeRef.desktop"
)

logger.info("Copying logos...")
shutil.copyfile("./zeeref/assets/logo.svg", "squashfs-root/logo.svg")
shutil.copyfile("./zeeref/assets/logo.png", "squashfs-root/.DirIcon")
...
run_command("./appimagetool.appimage", "squashfs-root", f"ZeeRef-{BEEVERSION}.appimage")
