...
# Create JSON with Linux libs needed for ZeeRef appimage
...
parser = argparse.ArgumentParser(
    description=("Create JSON with Linux libs needed for ZeeRef appimage")
)
parser.add_argument("pid", nargs=1, default=None, help="PID of running ZeeRef process")
parser.add_argument(
    "-l",
    "--loglevel",
    default="INFO",
...
parser.add_argument(
    "--jsonfile", default="linux_libs.json", help="JSON input/output file"
)
parser.add_argument(
    "--check-appimage",
    default=False,
...
def what_links_to(path):
    ...
    for filename in os.listdir(dirname):
        ...
        if os.path.islink(filename) and str(pathlib.Path(filename).resolve()) == path:
            links.add(filename)
